...
function renderBenoitYear(dataKey, opts = {}) {
  ...
  if (window.PRIV) {
    ...
    });
    {
      ...
      councilsTableHtml += `<tr class="tr">${isClotured ? '<td></td>' : ''}${hasRef ? '<td></td>' : ''}<td><strong>Total</strong></td><td class="a"><strong>${fmtPlain(totalHTEUR)}</strong></td>${ttcTotalCell2}<td></td><td class="a"><strong>${fmtPlain(totalDH)}</strong></td><td class="a"><strong>${fmtPlain(totalCommission)}</strong></td><td class="a"><strong>${fmtPlain(totalNetBenoit)}</strong></td><td></td></tr>`;
    }
    councilsTableHtml += `</tbody></table>`;
  }
