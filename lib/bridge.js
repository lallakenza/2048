...
(function (racine, fabrique) {
  ...
  function etatDesPreuves(DATA) {
    ...
    const compte = { verified: 0, missing: 0, 'n/a': 0, non_regle: 0, sans_bloc: 0 };
    const aTraiter = [];
    const vu = new Set();
    const visiter = (o) => {
      ...
      if (typeof o.ref === 'string' && (o.montant != null || o.htEUR != null || o.eur != null)) {
        if (!vu.has(o.ref)) {
          ...
          const st = r && r.paymentEvidenceStatus ? r.paymentEvidenceStatus : (marqueePayee ? 'sans_bloc' : 'non_regle');
          if (compte[st] != null) compte[st]++; else compte.sans_bloc++;
          if (st === 'missing' || st === 'sans_bloc') {
            aTraiter.push({ ref: o.ref, evidence: st, preuve: (r && r.preuve) || null, paymentDate: (r && r.paymentDate) || null });
          }
        }
      }
  ...
  function echeancesOuvertes(DATA) {
    ...
    const visiter = (o, contrepartie) => {
      ...
      const r = o.reglement;
      if (typeof o.ref === 'string' && r && r.status === 'pending') {
        out.push({
          ref: o.ref, contrepartie,
          montant: o.montant != null ? o.montant : o.htEUR,
          devise: 'EUR', echeance: o.dateDue || null, statut: 'en_attente',
          // Announced settlement date, if any — informative only, NOT counted as cash.
          paiementAnnonce: r.scheduledDate || null,
        });
      }
