...
/**
 * 11. No settlement — and no data freshness — dated in the future.
 *
 * CLT-UFA emails its payment advices a few days BEFORE the value date: the advice for
 * INVRTL019 arrived on 19/09/2026 announcing a payment on 23/09. Booking it as paid
 * on reception would have moved Augustin by 165 699 DH four days early, and pushed
 * `dataAsOf` into the future. An announced date belongs in `scheduledDate`, never in
 * `paymentDate`.
 */
function reglementsFuturs(DATA, aujourdhui) {
  ...
  const visiter = (o) => {
    ...
    if (Array.isArray(o)) { o.forEach(visiter); return; }
    const r = o.reglement;
    if (r && r.paymentDate && r.paymentDate > jour) {
      fautifs.push(`${o.ref} : paymentDate ${r.paymentDate} postérieure au ${jour} — utiliser scheduledDate`);
    }
    for (const v of Object.values(o)) if (v && typeof v === 'object') visiter(v);
  };
  visiter(DATA);
  const { derniereOperation } = require('./data-freshness.js');
  const asOf = derniereOperation(DATA).date;
  if (asOf && asOf > jour) fautifs.push(`dataAsOf ${asOf} postérieure au ${jour}`);
  return { ok: fautifs.length === 0, detail: fautifs };
}

module.exports = { reglementsFuturs, invariantsGains, allocationUnique, payeSansPreuve, sequencePerimetreEmetteur, fenetreTroisMois,
