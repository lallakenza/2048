...
const STATUTS_REGLEMENT = ['paid', 'pending'];
const PREUVES = ['verified', 'missing', 'n/a'];
...
function validerLot(factures, contexte, anomalies) {
  ...
  factures.forEach((f, i) => {
    ...
    if (estPayee(f)) {
      ...
      } else if (!STATUTS_REGLEMENT.includes(r.status)) {
        ajoute('erreur', 'reglement-statut-inconnu',
          `${ref} : statut « ${r.status} » hors du vocabulaire (${STATUTS_REGLEMENT.join(', ')})`);
      } else if (!PREUVES.includes(r.paymentEvidenceStatus)) {
        ajoute('erreur', 'preuve-statut-inconnu',
          `${ref} : paymentEvidenceStatus « ${r.paymentEvidenceStatus} » hors du vocabulaire (${PREUVES.join(', ')})`);
      } else if (r.paymentEvidenceStatus === 'verified') {
        // « verified » engage : il FAUT la date, le montant et la pièce.
        const manque = [];
        if (!r.paymentDate) manque.push('paymentDate');
        if (typeof r.montant !== 'number') manque.push('montant');
        if (!r.preuve) manque.push('preuve');
        if (manque.length) {
          ajoute('erreur', 'paye-sans-preuve',
            `${ref} est « verified » mais il manque ${manque.join(', ')} — passe en « missing » tant que la pièce n'est pas là`);
        }
      } else if (r.paymentEvidenceStatus === 'missing') {
        // Avertissement MAINTENU : l'absence de preuve est légitime, la masquer non.
        ajoute('avertissement', 'preuve-incomplete',
          `${ref} : encaissé sans preuve vérifiée${r.preuve ? ' — ' + r.preuve : ''}`);
      }
    }
